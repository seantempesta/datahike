...
- **Guarded merge with exact lineage** — `merge-db` / `merge-db!` accept an arg map `{:parents :tx-data :tx-meta :datahike/expected-basis-t}`; the merge writer is the ordinary `transact!` plus parents, so the basis fence and `:datahike/validate-report` refuse a merge before any datom or parent lands. Fixed: a transaction following a merge no longer inherits the merge's parents, and a merge batched with later transactions keeps its parents in the batch's one commit. *Experimental.* ([#TODO])

- **Query results shared by commit across branches and connections** — the query-result cache keys a committed value by `[store-id commit-id]` (plus an earlier numeric as-of time point), so every branch and connection of one store at one commit shares one bucket instead of recomputing and holding a duplicate. The scoped `[connection-id generation commit-id]` identity still owns lifetime: a put requires its generation admitted, each bucket records the scopes that stored or read it, single-flight coordinates per scope, and final release drops the scope from every bucket and evicts only buckets no admitted scope owns. Dependency-safe promotion stays within one connection generation, whose revisions share a baseline. Speculative, filtered and non-numeric temporal values still never cache. The bound is unchanged: `*query-cache-size*` buckets and `*query-cache-weight-limit*` weight. *Experimental.*
...
- **Transport-free authority capabilities** — `capabilities` now returns a bounded namespaced semantic catalog, `committed-value-identity` exposes exact attached value identity as ordinary data, and `q-with-evidence` preserves query results while reporting per-call cache and resource evidence. Addressable request cancellation wakes only the selected caller and signals shared work only after the final caller detaches. Exact cache-scope release returns bounded cleanup evidence. A demand-opened committed-report source performs constant-time generation-fenced offers at durable publication, fails closed on overflow, and is automatically abandoned at final connection release. *Experimental.* ([#TODO])
...
- **Pull dependency evidence** — `pull-with-evidence` and
  `pull-many-with-evidence` return their eager values with the same
  source-scoped dependency-plan shape used by queries.
  `pull-dependency-plan` parses selectors once through Datahike's PullSpec,
  includes lookup-ref attributes, canonicalizes reverse attributes, and widens
  wildcards to `:all` without retaining a database value. *Experimental.*
  ([#TODO])
...
- **`fork-database` — fork a database at a point in time** — new API operation copies the source store at the konserve layer and points the target's `:db` branch at the commit selected by `:at` (a tx-id long for an exact `:max-tx` match, an inst for the newest commit at or before it, or absent for the head), yielding a fully independent, writable database whose entity ids, transaction ids, tx-meta, schema and history are byte-faithful as of the fork point; new transactions continue from there. The target gets a fresh store identity (minted when the target config carries no `[:store :id]`) so source and fork connect side-by-side in one process, and `delete-database` on the fork never touches the source. Returns the target's effective config — connect with exactly that value. *Experimental.* ([#TODO])
...
- **Query planner: recursive-rule shortcuts assumed one recursion direction; disjoint-attrs hash-join multiplied duplicates unboundedly** — three defects in `execute-recursive-rule` and its join substrate. (1) The JVM-only shortcuts (`delta-driven-expand`, `magic-base-scan`) were not platform-gated: on CLJS the moment a simple binary transitive-closure rule's delta dropped below 16 tuples the shortcut returned `nil` and the fixpoint silently terminated mid-closure (a 19-edge chain returned exactly depth 5), and a ground call-arg threw `demand_set.size is not a function`; both are now `:clj`-gated (CLJS runs the plain semi-naive fixpoint). (2) Wherever the shortcuts DID run, they assumed the reverse recursion form `[?x :attr ?t] (rule ?t ?y)`; the equally-valid forward form `[?m :attr ?a] (rule ?m ?d)` (e.g. `descendant` over a parent ref) got a reverse expansion that truncated the closure the same way, and ground calls returned `#{}` in BOTH argument positions (even the reverse form broke with the ground arg at position 1). Lowering now derives each clause version's join topology structurally (`rec-expand-info` / `base-scan-info`); the expansion follows the clause's actual direction (forward EAVT / reverse AVET) and the magic-set path only fires for topologies its demand scheme is provably sound for (`magic-direction-ok?`) — everything else takes the plain fixpoint. New dynamic `datahike.query.execute/*fixpoint-shortcuts?*` (default true) forces the plain path for tests/forensics. (3) `hash-join` over relations with zero common attrs degenerated to a constant-key full Cartesian product, and rule/OR-branch unions carried duplicate tuples that multiply through subsequent joins — a 13-node graph manufactured 15.4M tuples from 783 distinct pairs (4GB+ heap, OOM). The no-common-attrs case now routes to an explicit deduped `prod-rel`, rule/OR unions are deduped (`distinct-rel` — Datalog relations are sets), and the fixpoint scopes the outer context's relations to those sharing vars with the rule's plans. Affected every CLJS consumer (planner always on) and JVM runs with `DATAHIKE_QUERY_PLANNER=true`. ([#TODO])
- **Query planner: cross-component split diverged from legacy on disjoint components and var-valued predicates** — (1) a disconnected where-component contributing no find or post-filter vars (e.g. `[?e _ _]` next to a rule on other vars) got an empty `:find` for its sub-query and silently collapsed the whole Cartesian merge to `#{}`; such components now project a witness var so a satisfiable component yields ≥1 row (and an unsatisfiable one correctly empties the result — reconciled with upstream's existence-constraint semantics on merge). (2) a cross-component post-filter whose fn position is a free var bound by a data clause (`[(?pred ?a)]` with `[_ :pred ?pred]`) threw "Cannot resolve predicate"; the predicate is now read per-tuple from the wide tuples, mirroring legacy context resolution. Affected every CLJS consumer (planner always on) and JVM runs with `DATAHIKE_QUERY_PLANNER=true`. ([#TODO])
- **Query planner: probe values collected from the wrong merge-datom field** — a probe var bound from a non-value slot of an entity-group merge clause (e.g. `?tx` from `[?e :attr _ ?tx]`) was collected as the merge datom's *value*, so a downstream join on that var (e.g. `[?tx :db/txInstant ?at]`) silently returned `#{}` when an id-lookup clause preceded the tx-binding clause — the exact order the "most selective clause first" guidance recommends. Affected every CLJS consumer (planner always on) and JVM runs with `DATAHIKE_QUERY_PLANNER=true`. `emit-tuple` now honors `collect-datom-field` on merge datoms. ([#TODO])
- **Proximum secondary restores from konserve on reopen instead of overwriting** — the `:proximum` factory now connect-if-exists: when its konserve store already holds a committed index it returns a passive restore skeleton (no `create-store`, no `:index/config`/`:branches` write) so `restore-secondary-indices` → `-sec-restore` → `load-commit` repopulates the HNSW from durable storage. Previously the factory unconditionally `create-store`d a fresh store for the reopen skeleton, which threw "store already exists" (file/jdbc) or collided in the JVM-global memory registry; restore was dropped and a `:db.secondary/only` vector attribute lost its values (the primary holds only a hash). Now a reopen genuinely restores — index live immediately, status `:ready`, no AEVT backfill. ([#TODO])
